...
export function renderTables(tables, containerId, actions) {
  ...
  if (tables.length === 0) {
    container.innerHTML = '<p class="text-gray-500 dark:text-gray-400 text-center py-8">Tidak ada tabel yang ditemukan.</p>';
    return;
  }
  ...
  tables.forEach((table, index) => {
    ...
    cardHeader.className = 'px-6 py-5 border-b border-slate-200 dark:border-slate-700 flex justify-between items-center bg-slate-50/50 dark:bg-slate-800/50 backdrop-blur-sm';
    
    const title = document.createElement('h3');
    title.className = 'text-lg font-semibold text-gray-800 dark:text-gray-100';
    ...
    btnEdit.className = 'px-4 py-2 text-sm font-semibold text-indigo-700 dark:text-indigo-400 bg-indigo-50/80 dark:bg-indigo-900/30 border border-indigo-100/50 dark:border-indigo-800/30 rounded-xl hover:bg-indigo-100 dark:hover:bg-indigo-900/50 transition-all flex items-center gap-1.5 btn-edit-mode';
    btnEdit.innerHTML = `
    ...
    btnInvoice.className = 'px-4 py-2 text-sm font-semibold text-white bg-gradient-to-r from-emerald-500 to-teal-500 dark:from-emerald-600 dark:to-teal-600 rounded-xl hover:from-emerald-600 hover:to-teal-600 dark:hover:from-emerald-500 dark:hover:to-teal-500 shadow-sm shadow-emerald-200 dark:shadow-none transition-all transform hover:-translate-y-0.5 flex items-center gap-1.5';
    ...
    btnDeleteTable.className = 'px-4 py-2 text-sm font-semibold text-rose-600 dark:text-rose-400 bg-rose-50/80 dark:bg-rose-900/30 border border-rose-100/50 dark:border-rose-800/30 rounded-xl hover:bg-rose-100 dark:hover:bg-rose-900/50 transition-all flex items-center gap-1.5';
    btnDeleteTable.innerHTML = `
      <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 6h18"/><path d="M19 6v14c0 1-1 2-2 2H7c-1 0-2-1-2-2V6"/><path d="M8 6V4c0-1 1-2 2-2h4c1 0 2 1 2 2v2"/><line x1="10" y1="11" x2="10" y2="17"/><line x1="14" y1="11" x2="14" y2="17"/></svg>
      Hapus Tabel
    `;
    btnDeleteTable.onclick = () => {
      showConfirm('Apakah Anda yakin ingin menghapus tabel ini?', () => {
    ...
    table.rows.forEach((row, rowIndex) => {
      ...
      tr.draggable = true;
      tr.className = 'cursor-grab hover:bg-slate-50/50 dark:hover:bg-slate-800/40 transition-colors';
      ...
      tr.addEventListener('dragover', (e) => {
        ...
        if (e.clientY - offset > 0) {
          tr.classList.add('border-b-4', 'border-indigo-400');
          tr.classList.remove('border-t-4');
        } else {
          tr.classList.add('border-t-4', 'border-indigo-400');
          tr.classList.remove('border-b-4');
        }
      });
      ...
      tr.addEventListener('drop', (e) => {
        ...
        if (draggedRow && draggedRow !== tr) {
          ...
          tableEl.dispatchEvent(new Event('input', { bubbles: true }));
        }
      });
      ...
      for (let i = 0; i < table.headers.length; i++) {
        ...
        td.className = 'px-4 py-3 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 font-medium';
      ...
      btnRowInvoice.className = 'text-emerald-600 dark:text-emerald-400 hover:text-emerald-800 dark:hover:text-emerald-300 text-sm font-bold flex items-center gap-1 shrink-0 px-2 py-1 bg-emerald-50 dark:bg-emerald-900/20 rounded-lg hover:bg-emerald-100 dark:hover:bg-emerald-900/40 transition-colors';
      ...
      btnDelete.innerHTML = `
        ...
        Hapus
      `;
      btnDelete.onclick = () => actions.onDeleteRow(table.tableId, tr);
